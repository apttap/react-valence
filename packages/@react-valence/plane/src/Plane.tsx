...
function Plane<T extends object>(
  ...
  const set = {
    size: {
      width: planer.width + planer.margin * 2,
      height: planer.height + planer.margin * 2,
    },
  };
  ...
  const updatePointerPos = (e) => {
    ...
    if (props.snapToGrid) {
      coords.x = Math.round((coords.x) / (gridSize / 2)) * (gridSize / 2);
      coords.y = Math.round((coords.y) / (gridSize / 2)) * (gridSize / 2);
    }
    ...
    if (pointerCoord.x > set.size.width / 2) {
      newFlagPosition.x = -1 * (pointerFlagTextWidth + 25);
    } else {
      newFlagPosition.x = 5;
    }

    if (pointerCoord.y > set.size.height / 2) {
  ...
  for (let rows = 0; rows <= props.gridSize; ++rows) {
    let posY = gridSize * rows + planer.margin;

    grid.push(
      <line
        x1={planer.margin}
        y1={posY}
        x2={planer.width + planer.margin}
        y2={posY}
    ...
    let posX = gridSize * rows + planer.margin;
  ...
  return (
    <div
      ...
      style={{ width: set.size.width, height: set.size.height }}
    >
      <div className={styles.Plane}>
        <svg
          viewBox={`0 0 ${set.size.width} ${set.size.height}`}
          ref={pointerSurface}
        >
          <defs>
          ...
          />
          <g transform="translate(10,11) scale(0.8)">
            <path fill="white" d="M30 30a1.5 1.5 0 0 1 -1.5 1.5H7.5a1.5 1.5 0 0 1 -1.5 -1.5v-13.5H1.5l15.491 -14.082a1.5 1.5 0 0 1 2.019 0L34.5 16.5h-4.5v13.5zm-13.5 -10.5v9h3v-9h-3z" />
          </g>
            ...
            >
              ...
              >
                ...
                <text
                  ...
                  ref={pointerFlagText}
                >
                  {(pointerCoord.x - planer.margin) * dimensionRatio.x}, {(pointerCoord.y - planer.margin) * dimensionRatio.x}
                </text>
              </motion.g>
            </motion.g>
          </g>
          ...
          />
          <g
            className={styles.CurrentPositionText}
            height={planer.margin}
            x={5}
            y={planer.width}
          >
            <text
              className={styles.CurrentPositionTextHeading}
              x={planer.margin}
              y={planer.width + planer.margin + 22}
            >
              Current Position
            </text>
            <text
              className={styles.CurrentPositionTextCoordinates}
              x={planer.margin}
              y={planer.width + planer.margin + 38}
            >
              {(selectedCoord.x - planer.margin) * dimensionRatio.x},{" "}
              {(selectedCoord.y - planer.margin) * dimensionRatio.y}
            </text>
          </g>
        </svg>
      </div>
